...
from .models import Instructor,Course,Student,Enrollment,Assignment,Submission,Announcement
import json
...
def get_user_details(request):
    ...
    # Iterate through each user and extract relevant details
...
@csrf_exempt
def student_dashboard_view(request, student_id):
    try:
        student = Student.objects.get(student_id=student_id)
        enrolled_courses = Enrollment.objects.filter(student__student_id=student_id)
        ...
        courses_list = [
            ...
            for course in Course.objects.filter(id=enrollment.course.id)
        ]
